...
- [蘋果開發者網站](https://idmsa.apple.com/IDMSWebAuth/signin?appIdKey=891bd3417a7776362562d2197f89480a8547b108fd934911bcbea0110d07f757&path=%2Faccount%2F&rv=1)
4. 必須使用 v3 版的 SDK

### Overview

1. 引入 SDK 並且初始化 APP_ID, APP_KEY, SERVER_TYPE                                                              //ok
2. 使用 `TPDirect.paymentRequestApi.checkAvailability()` 檢查瀏覽器是否可以用 Payment Request API                 //應該是瀏覽器及wallet那一段
3. 設置 Payment Request Data, 並且在 `supportedMethods` 填入 `["apple_pay"]`                                     //ok
4. 使用 `TPDirect.paymentRequestApi.setupApplePay()` 設置 Apple Pay 特有的參數 "Apple Pay merchant id"          //todo
...
設置 Apple Pay 特有的參數包含 `merchantIdentifier` `countryCode`                             //todo
